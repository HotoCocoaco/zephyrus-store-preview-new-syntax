...
public int Pets_Equip(int client, int itemid)
{
	...
	if(IsPlayerAlive(client))
	{
		...
		/*
		if(SF2_IsClientInGhostMode(client))
		{
			return 0;
		}
		*/
...
void CreatePet(int client)
{
	...
	if (StrContains(map, "ph_", false) != -1)	//match found
		return;
...
stock float GetClientDistanceToGround(int ent, int client, float pos2)
{
	float fOri[3];
	float fGround[3];
...
public void Store_OnPreviewItem(int client, char[] type, int index)
{
	...
	//放大缩小模型
	if (g_fScale[index] > 0.00 && g_fScale[index] != 1.00)
	{
		SetEntPropFloat(iPreview, Prop_Send, "m_flModelScale", g_fScale[index]);
	}
